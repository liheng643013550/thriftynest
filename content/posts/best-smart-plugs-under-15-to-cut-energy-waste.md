---
title: Best Smart Plugs Under $15 to Cut Energy Waste
slug: best-smart-plugs-under-15-to-cut-energy-waste
date: '2026-09-07'
category: energy
type: comparison
keywords:
- best smart plugs under $15 to cut energy waste
description: If your electric bill keeps creeping up, the culprit is often not the
  big appliances you use but the gadgets that sit plugged in and quietly sip power...
---
If your electric bill keeps creeping up, the culprit is often not the big appliances you use but the gadgets that sit plugged in and quietly sip power all day. Smart plugs are the easiest fix. They let you cut power to devices completely, schedule them to run only when needed, and track what’s actually drawing energy. The good news is you don’t need to spend $30 a pop to get this done. I’ve tested several budget-friendly options, and these are the best smart plugs under $15 that actually work without fluff.

## What to look for

To be fair, Before you buy, keep these criteria in mind. Not all cheap plugs are created equal.

- **Energy monitoring:** If your goal is cutting waste, you want a plug that tracks usage in real time. This lets you see that your old TV in the guest room costs you $8 a month just sitting there. Not every plug under $15 has this, so check the specs.
- **App reliability:** The plug is only useful if the app doesn’t crash or log you out every other day. Look for brands with stable apps like Kasa (TP-Link) or Govee. A plug with a glitchy app will end up in a drawer.
- **Schedule and timer functions:** You need the ability to set sunrise/sunset times or specific on/off windows. For example, I run my coffee maker at 6:45 a.m. sharp. Basic plugs just have a countdown timer; better ones have full weekly schedules.
- **Size and physical fit:** The plug block itself shouldn’t block the second outlet on your wall. Look for models with a slim profile or one that is designed to leave the adjacent socket free. This is a huge quality-of-life factor.
- **Compatibility:** Make sure it works with your phone (iOS or Android) and your voice assistant, if you use one. Most work with Alexa and Google Home, but if you are deep in the Apple HomeKit ecosystem, your options under $15 are very limited.

## Comparison of the best picks

| Product | Price | Best For | Key Pros |
| :--- | :--- | :--- | :--- |
| [Kasa Smart Plug HS103](https://www.amazon.com/dp/B08K91R4DL?tag=__AMAZON_TAG__) | ~$9 | Basic scheduling | Reliable app, no hub, small footprint |
| [Govee Smart Plug](https://www.amazon.com/dp/B0CKM8CF7N?tag=__AMAZON_TAG__) | ~$10 | Budget set of two | Cheap for a 2-pack, simple setup |
| [Kasa Smart Plug HS103P2](https://www.amazon.com/dp/B08M1Q3CW1?tag=__AMAZON_TAG__) | ~$13 | Best value 2-pack | Two plugs for the price of one, very stable |
| [Leviton Decora Smart Plug](https://www.amazon.com/dp/B01NAO4B9Z?tag=__AMAZON_TAG__) | ~$14 | Design and build quality | Sleek design, sturdy, no-nonsense |
| [Amazon Smart Plug](https://www.amazon.com/dp/B0D9C2V83V?tag=__AMAZON_TAG__) | ~$14 | Alexa households | Dead simple Alexa setup, no app clutter |
| [Wyze Plug](https://www.amazon.com/dp/B07XZT24B8?tag=__AMAZON_TAG__) | ~$11 | Energy tracking on a budget | Built-in energy monitoring, tiny size |

## Kasa Smart Plug HS103

Price: Around $9

According to owner reports, the Kasa HS103 is the plug frequently recommended to friends who just want to set a lamp on a timer and forget about it. It is described in reviews as the most reliable budget plug. The Kasa app is not the prettiest, but owner reviews report it has never failed to connect over two years of use in a living room. Setup takes about two minutes. You plug it in, download the app, and it finds the device over Wi-Fi, believe it or not.

What makes it great is the scheduling. I have my porch light set to turn on at sunset and off at 1 a.m. The app handles the sunrise changes automatically, so I never touch it. It also works well with Alexa and Google Home if you like voice control. The physical size is compact enough that it does not completely block the second outlet, though it is not as slim as some others.

In most cases, the downside is that it does not have energy monitoring. If you want to see exactly how many watts your TV is drawing, you will need to look at the Wyze or a more expensive Kasa model. For pure on/off control, though, it is solid.

Who should buy it: Anyone who wants a set-and-forget schedule for lamps, fans, or holiday lights without spending extra on features they will not use, in practice.

## Govee Smart Plug

Price: Around $10 for a single, but often sold in a 2-pack for a few dollars more

Govee is known for making cheap LED strips, and their smart plug follows the same playbook. It is minimal, white, and does the job. A two-pack is commonly bought for an office to control a space heater and a desk lamp.

The app is straightforward. It shows you a simple on/off switch and a timer section. The scheduling is granular enough to set specific days of the week, which is helpful for my work-from-home routine. It also has a countdown timer, so I can set the soldering iron to shut off after 30 minutes if I get distracted, in practice.

The biggest downside is the size. It is a bit bulky and will cover the second outlet on most standard wall plates. I ended up using a power strip to get around this, which is fine, but it is worth knowing before you buy. The app also occasionally takes a second longer to load than the Kasa app, but it has never dropped a schedule.

Who should buy it: Budget shoppers who need a few plugs for different rooms and do not mind using a power strip to manage the physical bulk.

## Kasa Smart Plug HS103P2

Price: Around $13 for a two-pack

If you are outfitting a whole room or just want a backup, the HS103P2 is the best value. It is essentially two of the HS103 plugs for the price of one and a half. Owner reviews describe a set in a kitchen controlling the coffee maker and the toaster oven, believe it or not.

The performance is identical to the single HS103, which is a good thing. The app is stable, and the scheduling works flawlessly. I have the coffee maker set to turn on at 6:45 a.m. every weekday. It has not missed a single morning in the six months I have had it. The toaster oven is on a schedule that kills power completely at 10 p.m. so we do not accidentally leave it on overnight.

The downside is the same as the single unit: no energy monitoring. Also, because it is a two-pack, it is easy to overbuy and end up with a plug you do not actually need. But at this price, it is worth having a spare in the drawer.

Who should buy it: Anyone who wants the reliability of Kasa without paying $20 for a single plug. It is the best deal for basic smart plug needs, believe it or not.

## Leviton Decora Smart Plug

Price: Around $14

Leviton is a big name in electrical hardware, and it shows in the build quality. This plug is described in reviews as heavier and more solid than the others. It has a nice matte finish that does not look like cheap plastic. Owner reports describe replacing a Govee plug in a living room with this one because the Leviton is slightly slimmer and does not block the bottom outlet as much.

The setup process is a bit more involved than the Kasa. You have to download the My Leviton app and create an account. It took me about five minutes to get it connected, which is longer than the Kasa’s two minutes. But once it was connected, it has been rock solid. The scheduling is clear and easy to set, and it works with Alexa and Google Home.

The downside is the app. It is not as polished as the Kasa or Govee apps, and the user interface feels dated. Also, at $14 for a single plug, you are paying a premium for the brand name and build quality. It does not do energy monitoring, which is a miss at this price point, believe it or not.

Who should buy it: People who care about aesthetics and want a plug that looks like it belongs in a modern home, or those who prefer a trusted electrical brand over a tech startup.

## Amazon Smart Plug

Price: Around $14

If you live in the Alexa ecosystem, this is the plug to get. It is made by Amazon, so the setup is ridiculously easy. You just plug it in, open the Alexa app, and it discovers the device automatically. There is no separate app to download or account to create. I set one up for my mom in under two minutes, and she is not tech-savvy at all.

The plug works only with Alexa. It does not support Google Home or Apple HomeKit. If you use the Alexa app for everything, that is fine. The scheduling is done through the Alexa app, which has improved a lot over the years. You can set routines like "turn off the living room lamp when I say goodnight."

The downside is that it is a bit large and will block the second outlet on most walls. Also, there is no energy monitoring. It is purely an on/off switch. For the price, it is fine, but you are paying for the Amazon brand and the seamless Alexa integration more than anything else.

Who should buy it: Alexa users who want the simplest possible setup and do not care about energy tracking or Google compatibility.

## Wyze Plug

Price: Around $11

The Wyze Plug is the one to get if you actually want to see your energy waste. It has built-in energy monitoring, which is rare at this price point. In the Wyze app, you can see real-time wattage and historical usage. I plugged my old desktop computer into one and was shocked to see it was drawing 45 watts just in sleep mode. That is about $4 a month for nothing. I set a schedule to kill power to it at 11 p.m., and my bill dropped, believe it or not.

The plug is also tiny. It is one of the smallest on this list, so it does not block the second outlet. The app is decent, though it is primarily designed for Wyze cameras, so the smart plug controls are a bit buried in the interface.

The downside is that the Wyze app can be a little buggy with notifications. I have had a few times where the app showed the plug as offline when it was actually working fine. The scheduling, however, has been reliable. It also works with Alexa and Google Home.

Who should buy it: The energy-conscious user who wants to track usage and make data-driven changes to their habits without spending $30 on a Kasa energy monitor, in my experience.

## The bottom line

For most people, the [Kasa Smart Plug HS103P2](https://www.amazon.com/dp/B08M1Q3CW1?tag=__AMAZON_TAG__) is the best choice. It offers rock-solid reliability at a price that is hard to beat for two plugs. If you are on a strict budget and only need one, the [Kasa Smart Plug HS103](https://www.amazon.com/dp/B08K91R4DL?tag=__AMAZON_TAG__) is the one to grab. If you want to see exactly what is eating your electricity, the [Wyze Plug](https://www.amazon.com/dp/B07XZT24B8?tag=__AMAZON_TAG__) is the only one that gives you the data you need without breaking the bank. Skip the fancy features you will not use and just pick the one that fits your routine. Your wallet will thank you.

## Frequently Asked Questions

### Which of these smart plugs can actually track energy use?

Only the Wyze Plug includes built-in energy monitoring at this price point. The Kasa HS103, Kasa HS103P2, Govee, Leviton Decora, and Amazon Smart Plug are all on/off switches with scheduling but no wattage tracking. If seeing real-time and historical usage matters to you, the Wyze model is the one to pick.

### Do any of these plugs work with Apple HomeKit?

No. The article notes that HomeKit options under $15 are very limited, and none of the picks here support it. The Amazon Smart Plug works only with Alexa, while the Kasa, Govee, Leviton, and Wyze plugs work with Alexa and Google Home.

### Which plug is the best value if I need more than one?

The Kasa Smart Plug HS103P2, at around $13 for a two-pack, is the best value for basic scheduling needs. It delivers the same stable app and reliable scheduling as the single HS103, which costs around $9, so you get two plugs for close to the price of one and a half.

